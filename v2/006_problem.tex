\documentclass{article}
\include{header}

\begin{document}
\rating{2}{2}
Let $f_{n,m}\colon [n] \rightarrow [m]$ be a uniformly random function, and
consider the convex hull around the points
$\{(1, f_{n,m}(1)), \hdots (n, f_{n,m}(n))\}$ in $\mathbb Z^2$.
\begin{figure}[!h]
  \centering
  \begin{tikzpicture}
    \draw[gray, dashed] (0, 0) grid (2, 1);
    \draw[red, very thick] (0, 0) -- (1, 0) -- (2, 0);
  \end{tikzpicture}\hspace{0.5cm}
  \begin{tikzpicture}
    \draw[gray, dashed] (0, 0) grid (2, 1);
    \draw[red, very thick] (0, 0) -- (1, 0) -- (2, 1) -- (0, 0);
  \end{tikzpicture}\hspace{0.5cm}
  \begin{tikzpicture}
    \draw[gray, dashed] (0, 0) grid (2, 1);
    \draw[red, very thick] (0, 0) -- (1, 1) -- (2, 0) -- (0, 0);
  \end{tikzpicture}\hspace{0.5cm}
  \begin{tikzpicture}
    \draw[gray, dashed] (0, 0) grid (2, 1);
    \draw[red, very thick] (0, 0) -- (1, 1) -- (2, 1) -- (0, 0);
  \end{tikzpicture}\hspace{0.5cm}\\~\\
  \begin{tikzpicture}
    \draw[gray, dashed] (0, 0) grid (2, 1);
    \draw[red, very thick] (0, 1) -- (1, 0) -- (2, 0) -- (0, 1);
  \end{tikzpicture}\hspace{0.5cm}
  \begin{tikzpicture}
    \draw[gray, dashed] (0, 0) grid (2, 1);
    \draw[red, very thick] (0, 1) -- (1, 0) -- (2, 1) -- (0, 1);
  \end{tikzpicture}\hspace{0.5cm}
  \begin{tikzpicture}
    \draw[gray, dashed] (0, 0) grid (2, 1);
    \draw[red, very thick] (0, 1) -- (1, 1) -- (2, 0) -- (0, 1);
  \end{tikzpicture}\hspace{0.5cm}
  \begin{tikzpicture}
    \draw[gray, dashed] (0, 0) grid (2, 1);
    \draw[red, very thick] (0, 1) -- (1, 1) -- (2, 1);
  \end{tikzpicture}

  \caption{
    Examples of $f_{3,2}$. Here the expected number of sides on a convex
    hull is $2.75$
  }
\end{figure}

\begin{question}
  What is the probability of seeing a $k$-gon (for some fixed $k$), when given
  a uniformly random function $f_{n,m}$?
\end{question}

\begin{related}
  \item What value of $k$ has the highest probability?
  \item What is the expected value of the number of sides?
  \item What if $f_{n,n}$ is restricted to be a permutation?
  \item What if $f_{n,m}$ is injective?
\end{related}
\end{document}
