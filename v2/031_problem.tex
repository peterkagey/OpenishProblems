...
\begin{figure}[!h]
  ...
  \begin{multicols}{2}\begin{enumerate}[(a)]
    % 1
    \item
    \begin{tikzpicture}[scale=0.7, baseline=6]
    ...
    \begin{tikzpicture}[scale=0.7, baseline=6]
      ...
      \foreach \x in {0,...,1} {\draw[gray, thick] (1.5*\x, 0) rectangle (1.5*\x + 1, 1);}
      \foreach \x in {1,...,1} {\node at (1.5*\x - 0.4,0) {\Large,};}
      \draw [thick,decorate,decoration={brace,amplitude=3pt}] (-0.1,-0.1)--(-0.1,1.1);
      \draw [thick,decorate,decoration={brace,amplitude=3pt,mirror}] ({1.5 + 1.1},-0.1)--({1.5 + 1.1},1.1);
    \end{tikzpicture}
    % 3
    \item
    \begin{tikzpicture}[scale=0.7, baseline=6]
